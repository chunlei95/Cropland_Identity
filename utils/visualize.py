...
def visualize(image, result, color_map, save_dir=None, weight=0.6):
    ...
    Args:
        ...
        result (np.ndarray | str): The predict result of image.
    ...
    """
    if type(result) == str and os.path.exists(result):
        # 读取分割图，cv2.imread读不了中文路径，此处使用cv2.imdecode的方式读取
        result = cv2.imdecode(np.fromfile(result, dtype=np.uint8), cv2.IMREAD_GRAYSCALE)
        result_ = result.copy()
        result[result_ == 255] = 1
