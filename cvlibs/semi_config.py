...
class SemiConfig(object):
    '''
    Semi-Supervised Training configuration parsing. The only yaml/yml file is supported.
                ...
                for item in args['types']:
                    if item['type'] != 'MixedLoss':
                        if 'ignore_index' in item:
                            assert item[
                                       'ignore_index'] == self.train_dataset.ignore_index, 'If ignore_index of loss is set, ' \
                                                                                           ...
                                                                                           'train_dataset by default.'.format(
    ...
    @property
    def unlabeled_train_dataset_class(self) -> Generic:
        dataset_type = self.unlabeled_train_dataset_config['type']
        return self._load_component(dataset_type)
    ...
    @property
    def unlabeled_train_dataset(self) -> paddle.io.Dataset:
        _unlabeled_train_dataset = self.unlabeled_train_dataset_config
        if not _unlabeled_train_dataset:
            return None
        return self._load_object(_unlabeled_train_dataset)
