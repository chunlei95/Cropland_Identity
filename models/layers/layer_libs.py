...
class Encoder(nn.Layer):
    def __init__(self, stage_channels, depth, norm_type=nn.LayerNorm, act_type=nn.GELU):
        super().__init__()
        self.stage_num = len(stage_channels)
        if type(depth) == int:
            self.stage_depth = [depth] * self.stage_num
        elif type(depth) in [list, tuple]:
            assert len(depth) == self.stage_num
            self.stage_depth = depth
        else:
            raise RuntimeError(
                f'the type of depth is not correct, expect int or list or tuple, but received {type(depth)}!')
...
class Decoder(nn.Layer):
    def __init__(self, stage_channels, depth, norm_type=nn.LayerNorm, act_type=nn.GELU):
        super().__init__()
        self.base_channels = stage_channels[0]
        self.stage_in_channels = stage_channels[1:]
        self.stage_in_channels.reverse()
        self.stage_channels = stage_channels[:-1]
        self.stage_channels.reverse()
        self.stage_num = len(self.stage_channels)
        if type(depth) == int:
            self.stage_depth = [depth] * self.stage_num
        elif type(depth) in [list, tuple]:
            assert len(depth) == self.stage_num
...
class BasicConvBlock(nn.Layer):
    def __init__(self, in_channels, out_channels=None, kernel_size=3, norm_type=nn.LayerNorm, act_type=nn.GELU):
        super().__init__()
